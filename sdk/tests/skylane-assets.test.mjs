...
const cliPath = path.join(repoRoot, "sdk", "packages", "skylane", "cli.mjs");
...
function createFakeSystemCommands(t, options = {}) {
  ...
  const binDir = createTempDir(t, "skylane-cli-bin-");
...
const outputPath = process.env.SKYLANE_TEST_PS_OUTPUT_PATH;
if (outputPath && fs.existsSync(outputPath)) {
  process.stdout.write(fs.readFileSync(outputPath, "utf8"));
}
process.exit(Number(process.env.SKYLANE_TEST_PS_EXIT_CODE ?? "0"));
`);

  writeExecutable(path.join(binDir, "open"), `#!${process.execPath}
const fs = require("node:fs");
const args = process.argv.slice(2);
const logPath = process.env.SKYLANE_TEST_OPEN_LOG_PATH;
...
`);
  ...
  return {
    binDir,
    env: {
      SKYLANE_TEST_PS_OUTPUT_PATH: psOutputPath,
      SKYLANE_TEST_PS_EXIT_CODE: psExitCode,
      SKYLANE_TEST_OPEN_LOG_PATH: openLogPath,
      SKYLANE_TEST_FAILING_OPEN_APPS: JSON.stringify([...failureSet]),
    },
    openLogPath,
  };
}
...
test("CLI build copies local assets into .skylane/build/assets", async (t) => {
  const widgetDir = createTempDir(t, "skylane-cli-assets-copy-");
  ...
  assert.equal(
    fs.readFileSync(path.join(widgetDir, ".skylane", "build", "assets", "covers", "hero.txt"), "utf8"),
    "cover-art"
  );
});

test("CLI build removes stale copied assets when the source assets directory disappears", async (t) => {
  const widgetDir = createTempDir(t, "skylane-cli-assets-prune-");
  const sourceAssetsDir = path.join(widgetDir, "assets");
  const builtAssetPath = path.join(widgetDir, ".skylane", "build", "assets", "icon.txt");
  writeWidgetFixture(widgetDir, `
    export default function Widget() {
      return null;
    }
...
test("CLI build preserves the last good bundle when asset copying fails", async (t) => {
  const widgetDir = createTempDir(t, "skylane-cli-assets-staging-");
  const sourceAssetsDir = path.join(widgetDir, "assets");
  const builtBundlePath = path.join(widgetDir, ".skylane", "build", "index.cjs");
  const builtAssetPath = path.join(widgetDir, ".skylane", "build", "assets", "icon.txt");
...
test("CLI dev rebuilds when assets are added after startup", async (t) => {
  const widgetDir = createTempDir(t, "skylane-cli-assets-dev-");
  const fakeHome = createTempDir(t, "skylane-cli-home-");
  const builtBundlePath = path.join(widgetDir, ".skylane", "build", "index.cjs");
  const builtAssetPath = path.join(widgetDir, ".skylane", "build", "assets", "late.txt");
...
test("CLI dev notifies each unique running Skylane bundle path", async (t) => {
  const widgetDir = createTempDir(t, "skylane-cli-dev-fanout-");
  const fakeHome = createTempDir(t, "skylane-cli-home-");
  const builtBundlePath = path.join(widgetDir, ".skylane", "build", "index.cjs");
  const bundleA = "/Applications/Skylane.app";
  const bundleB = "/Users/test/DerivedData/Build/Products/Debug/Skylane.app";
  const commands = createFakeSystemCommands(t, {
    psOutput: [
      `${bundleA}/Contents/MacOS/Skylane`,
      `${bundleA}/Contents/MacOS/Skylane --launched-by-test`,
      `${bundleB}/Contents/MacOS/Skylane`,
  ...
  assert.match(stdout, /Notified 2 running Skylane installation\(s\)\./);
});

test("CLI dev warns and keeps watching when no Skylane process is running", async (t) => {
  const widgetDir = createTempDir(t, "skylane-cli-dev-no-app-");
  const fakeHome = createTempDir(t, "skylane-cli-home-");
  const builtBundlePath = path.join(widgetDir, ".skylane", "build", "index.cjs");
  const commands = createFakeSystemCommands(t, {
    psOutput: "",
  ...
  await waitFor(() => fs.existsSync(builtBundlePath), 10000);
  await waitFor(() => stderr.includes("no running Skylane installations found"), 10000);

  assert.equal(readOpenLog(commands.openLogPath).length, 0);
  assert.match(stdout, /Built tmp\.widget ->/);
  assert.match(stderr, /Warning: no running Skylane installations found; no app was notified\./);
});

test("CLI dev continues notifying other bundles when one delivery fails", async (t) => {
  const widgetDir = createTempDir(t, "skylane-cli-dev-partial-failure-");
  const fakeHome = createTempDir(t, "skylane-cli-home-");
  const builtBundlePath = path.join(widgetDir, ".skylane", "build", "index.cjs");
  const goodBundle = "/Applications/Skylane.app";
  const badBundle = "/Users/test/DerivedData/Build/Products/Debug/Skylane.app";
  const commands = createFakeSystemCommands(t, {
    psOutput: [
      `${goodBundle}/Contents/MacOS/Skylane`,
      `${badBundle}/Contents/MacOS/Skylane`,
    ].join("\n"),
  ...
  assert.match(stdout, /Notified 1 running Skylane installation\(s\)\./);
